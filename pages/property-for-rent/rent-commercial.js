...
export default function rentcommercial() {
    ...
    const { data, isLoading, isError } = fetcher('search/rent-commercial')
...
function Ads({ data }) {
    const { id, identity, images, breadcrumb, title, currency, price, location, category_type, bedrooms, bathrooms, area } = data;
    return (
        <ListingCard key={id} id={id} identity={identity} images={images} breadcrumb={breadcrumb} title={title} currency={currency} price={price} location={location} category_type={category_type} bedrooms={bedrooms} bathrooms={bathrooms} area={area} />
    )
}