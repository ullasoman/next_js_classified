...
export default function numberplates() {
    ...
    const { data, isLoading, isError } = fetcher('search/number-plates')
...
function Ads({ data }) {
    const { id, identity, breadcrumb, title, title_for_vue_url, images, currency, price, location } = data;
    return (
        <ListingCard key={id} id={id} identity={identity} directory={'number-plates'} breadcrumb={breadcrumb} title={title} title_for_vue_url={title_for_vue_url} images={images} currency={currency} price={price} location={location} />
    )
}