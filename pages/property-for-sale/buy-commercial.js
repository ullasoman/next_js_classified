...
export default function boats() {
    ...
    const { data, isLoading, isError } = fetcher('search/sale-commercial')
...
function Ads({ data }) {
    const { id, identity, breadcrumb, title, images, currency, price, category_type, location, area } = data;
    return (
        <ListingCard key={id} id={id} identity={identity} breadcrumb={breadcrumb} title={title} images={images} currency={currency} price={price} category_type={category_type} location={location} area={area} />
    )
}