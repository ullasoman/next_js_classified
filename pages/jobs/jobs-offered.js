...
export default function jobsoffered() {
    ...
    const { data, isLoading, isError } = fetcher('search/job-hire')
...
function Ads({ data }) {
    const { id, identity, breadcrumb, title, company, commitment, expected_salary, work_experience, education_level, location } = data;
    return (
        <JobListingCard key={id} id={id} identity={identity} breadcrumb={breadcrumb} title={title} company={company} commitment={commitment} expected_salary={expected_salary} work_experience={work_experience} education_level={education_level} location={location} />
    )
}